...
hostname="glatt-client-hypervisor"
currentuser="$( whoami)"
...
echo
echo " +---------------------------------------------------+"
echo " |       UNATTENDED GLATT HYPERVISOR ISO MAKER       |"
echo " +---------------------------------------------------+"
echo
...
if [[ ! -f $tmp/$seed_file ]]; then
    ...
    download "https://raw.githubusercontent.com/djcloutier/gch/master/$seed_file"
fi
...
# set late command
   ...
   late_command="chroot /target curl -L -o /home/$username/startup.sh https://raw.githubusercontent.com/djcloutier/gch/master/startup.sh ;\
     chroot /target chmod +x /home/$username/startup.sh ;"
...
sed -i "/label install/ilabel autoinstall\n\
  menu label ^Autoinstall Glatt Client Hypervisor\n\
...
cd $tmp/iso_new
(mkisofs -D -r -V "GCH" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o $tmp/$new_iso_name . > /dev/null 2>&1) &
spinner $!
